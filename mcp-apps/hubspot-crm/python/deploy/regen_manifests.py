"""Regenerate mcp-tools.json + ai-plugin.json from the live HS server.

Writes the manifests, then validates what was written. Exits 1 on drift —
LocalDeploy.ps1 / ServerDeploy.ps1 honor that and abort before MOS3 upload.

Run from hubspot-crm/python/:
    python deploy/regen_manifests.py

Honors MCP_GATEWAY_URL: LocalDeploy sets it to the tunnel URL, ServerDeploy
sets it to the ACA FQDN.
"""
import json
import os
import re
...
# Ensure the app root is on sys.path so hs_crm_mcp is importable
...
import hs_crm_mcp.hubspot_server as hs  # noqa: E402

SERVER_PATH    = "/mcp"
TOOL_PREFIX    = "hs__"
TUNNEL_BASE    = os.getenv("MCP_GATEWAY_URL", "https://localhost:8082")
...
def main() -> int:
    entries, functions = build_entries(hs)
